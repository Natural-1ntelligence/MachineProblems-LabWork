                            ...
                            queue<TreeNode*>& mergeQueue)
{
  ...
  //  smallest = (singleQueue.front().freq > mergeQueue.front().freq) ? mergeQueue.front() : singleQueue.front();
    //smallest.pop();
    //return smallest;
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)  //DONE
{
     ...
     */
     for(auto it = frequencies.begin(); it!=frequencies.end(); it++){
         TreeNode* current = new TreeNode(*it);
     ...
     while(!(mergeQueue.size() == 1 && singleQueue.empty())){
         ...
         l = removeSmallest(singleQueue, mergeQueue);
         r = removeSmallest(singleQueue, mergeQueue);

         TreeNode* current = new TreeNode((l->freq).getFrequency()+(r->freq).getFrequency());
         current->left = l;
         current->right = r;
         mergeQueue.push(current);
      }
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)  //DONE
{
    ...
    while (bfile.hasBits()) {
        ...
        if (!(bfile.hasBits()) && current->left == NULL){
            ss<<(current->freq).getCharacter();
            current = root_;
            continue;  }
...
void HuffmanTree::writeTree(BinaryFileWriter& bfile)  // DONE
{
    writeTree(root_, bfile);
    bfile.close();   //watch for errors
}

void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)  //DONE
{
    /**
     * @todo Your code here!
     *
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)  //DONE
{
    /**
     * @todo Your code here!
     *
  ...
  TreeNode * newn;
 	if (!bfile.hasBits()) {return NULL; }
 		...
 		newn = new TreeNode(new_freq); 	  }
 	else {
 		Frequency new_freq(0);
 		newn = new TreeNode(new_freq);
 		newn->left = readTree(bfile);
 		newn->right = readTree(bfile); 	 }
