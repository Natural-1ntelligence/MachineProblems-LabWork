...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
        ...
        double luminanceEdit = 1 - (dist*.005);
        pixel.l = pixel.l * luminanceEdit;
      }}
  return image;
...
PNG illinify(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
        ...
        double distToOragnge = fmin(abs(pixel.h - 11), abs(360 - pixel.h + 11));
        double distToBlue = fmin(abs(pixel.h - 211),abs(360-211+pixel.h));
        if (distToOragnge>distToBlue) {pixel.h = 216;}
        else {pixel.h = 11;}
      }}
  return image;
}
...
PNG watermark(PNG firstImage, PNG secondImage) {
 ...
 for (unsigned x = 0; x < firstImage.width(); x++) {
    for (unsigned y = 0; y < firstImage.height(); y++) {
        ...
        if (pixel2.l == 1) {
          if (pixel1.l>0.8) {pixel1.l = 1;}
          else {pixel1.l = pixel1.l+0.2;}
