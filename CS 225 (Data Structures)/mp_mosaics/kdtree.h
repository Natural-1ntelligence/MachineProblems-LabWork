...
class KDTree
{
  ...
  public:
    ...
    void sort(vector<Point<Dim>>& new_Points, int dim);  //added

    void makeTree(KDTreeNode*& N, vector<Point<Dim>> new_Points, int d); //added

    void destroy_(KDTreeNode * N);  //added

    void find(KDTreeNode* N, Point<Dim> &currentBest,Point<Dim> &q,  int d) const;  //added
    /**
