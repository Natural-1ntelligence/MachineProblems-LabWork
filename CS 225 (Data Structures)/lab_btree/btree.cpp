...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     */
     ...
     if (subroot->is_leaf == true){return V(); }
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
  ...
  new_right->elements.assign(mid_elem_itr+1, child->elements.end());  //first right, then left
  if(!child->is_leaf) { new_right->children.assign(mid_child_itr, child->children.end()); }
  new_left->elements.assign(child->elements.begin(), mid_elem_itr);
  if(!child->is_leaf){ new_left->children.assign(child->children.begin(), mid_child_itr); }
