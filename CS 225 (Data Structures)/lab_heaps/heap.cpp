...
{
    // @TODO Update to return the index of the child with highest prioritytv
    ///   as defined by higherPriority(
  /*  if (rightChild(currentIdx) >= _elems.size() && leftChild(currentIdx) >= _elems.size()) { return 0; }
    ...
    if (_elems.size() == leftChild(currentIdx)+1) {return leftChild(currentIdx);}
    if (higherPriority(_elems[leftChild(currentIdx)], _elems[rightChild(currentIdx)])) { return leftChild(currentIdx); }
    else { return rightChild(currentIdx); }
}
...
void heap<T, Compare>::heapifyDown(size_t currentIdx)
{
  ...
  /*  if(hasAChild(currentIdx)){
    ...
    return;  */ //end recursing when at leaf or when priority is satisfied
...
heap<T, Compare>::heap(const std::vector<T>& elems)
{
   ...
   for(size_t j = parent(_elems.size() - 1); j >= root(); j--) { heapifyDown(j); }
}
...
void heap<T, Compare>::updateElem(const size_t & idx, const T& elem)
{
    ...
    _elems[idx + 1] = elem; //+1 since idx starts at 0
    for(size_t i = root(); i < _elems.size(); i++)  { heapifyDown(i); }
}
...
bool heap<T, Compare>::empty() const
{
    ...
    if (_elems.size() == 1)  { return true; }  //ERROR??
    else { return false; }
}
