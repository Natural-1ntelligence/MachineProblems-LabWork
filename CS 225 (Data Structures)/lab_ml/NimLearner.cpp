...
std::vector<Edge> NimLearner::playRandomGame() const {
  ...
  Vertex current = "p1-"+to_string(_startingTokens);
  while(current!="p1-0" && current != "p2-0"){
    vector<Vertex> adj = g_.getAdjacent(current);
    Vertex temp = adj[rand()%adj.size()];
...
void NimLearner::updateEdgeWeights(const std::vector<Edge> & path) {
    ...
    if(path.back().dest == "p2-0"){
      if(ed.source.substr(1, 1) == "1"){
        g_.setEdgeWeight(ed.source, ed.dest, g_.getEdgeWeight(ed.source, ed.dest) + 1);
      }
      else{
        g_.setEdgeWeight(ed.source, ed.dest, g_.getEdgeWeight(ed.source, ed.dest) - 1);
      }
