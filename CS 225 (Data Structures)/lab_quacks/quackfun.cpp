...
while (!input.empty()) {
  ...
  if (input.front() == '[') {count.push(input.front());}
  else if (input.front() == ']') {count.pop();}
  input.pop();
  }
...
bool verifySame(stack<T>& s, queue<T>& q){
   ...
   while (!s2.empty()) {
     if (s2.top() != q.front()) {return false;}
     s2.pop();
     q.pop();
   }
