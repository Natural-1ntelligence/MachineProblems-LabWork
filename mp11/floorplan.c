...
int is_in_subtree(node_t* a, node_t* b) {
  if (b==NULL|| a==b)
    ...
    return 0;
  else if (is_in_subtree(a->right,b)||is_in_subtree(a->left,b))
    return 1;
  else
    return 0;
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  if(a->parent->left==a)      //logic to switch the parent's left or right
    tmp1=a->parent->left;
  ...
  a->parent=b->parent;      //switch the parents
  b->parent=tmp2;
